...
async def test_downloader_integration(make_data):
    ...
    event_ids = pd.DataFrame({
            ...
            'DESCRIPTION': ["McDaniels 25' 3PT Jump Shot (3 PTS) (Randle 1 AST)"],
            'FILE_PATH': [""],
    })
    ...
    # make file path
    file_path = os.path.join(data_dir, "{}.mp4".format(row.EVENTNUM))
    ...
    assert event_ids.loc[event_ids['EVENTNUM'] == 8, 'FILE_PATH'].values[0] != "", "The FILE_PATH should not be empty."
